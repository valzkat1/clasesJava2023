 ...
 */
public class JFrameVistaPrincipal extends TemlateJframe {
    ...
    public void listarContactos(Collection<Contactos> contactos) {
        limpiarFilas();
        for (Contactos c : contactos) {
            agregarFila(c.getId(), c.getNombre(), c.getApellidos(), c.getEmail());
        }
    }
